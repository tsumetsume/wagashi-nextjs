...
test.describe("和菓子シミュレーター画面", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // APIレスポンスを待機
    await page.waitForResponse("**/api/sweets")
    await page.waitForResponse("**/api/box-types")
    await page.waitForResponse("**/api/stores/store-1")
  })
  ...
  test("シミュレーター画面が正しく表示される", async ({ page }) => {
    ...
    // ボックスエリアの表示確認
    await expect(page.locator('[data-testid="box-area"]').first()).toBeVisible()
    
    // 商品選択エリアの表示確認
    await expect(page.locator('[data-testid="selection-area"]')).toBeVisible()
    
    // 和菓子アイテムの表示確認
    await expect(page.getByText("桜餅")).toBeVisible()
    await expect(page.getByText("どら焼き")).toBeVisible()
  })

  test("和菓子をドラッグ&ドロップで配置できる", async ({ page }) => {
    // 桜餅が表示されるまで待機
    const sakuraMochi = page.locator('[data-testid="sweet-item-sweet-1"]')
    await expect(sakuraMochi).toBeVisible()
    ...
    await expect(page.locator('[data-testid="placed-item"]').first()).toBeVisible()
    
    // どら焼きも配置
    const dorayaki = page.locator('[data-testid="sweet-item-sweet-2"]')
    await expect(dorayaki).toBeVisible()
  ...
  test("和菓子を回転できる", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="context-menu-rotate"]')).toBeVisible()
  ...
  test("和菓子をダブルクリックで詳細モーダルが表示される", async ({ page }) => {
    ...
    await expect(page.getByText("桜餅")).toBeVisible()
    await expect(page.getByText("春の代表的な和菓子")).toBeVisible()
    await expect(page.getByText("200円")).toBeVisible()
